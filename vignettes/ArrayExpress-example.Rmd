...
We can see that the four signatures that are most strongly active in wildtype
...
```{r}
similar_sigs <- c("Node34neg", "Node28pos", "Node50pos", "Node28neg",
                  "Node34pos", "Node41pos", "Node49neg", "Node65pos")
...
plot_marginal_activation(marginal_limma_result = marginal_limma,
                         signature_order = similar_sigs)
```

Among Node50pos, Node28neg, Node34pos, and Node41pos,
Node50pos is the least unique signature, as it
...
Between Node34neg and Node28pos, we will choose Node28pos.
Node49neg and Node65pos do not cover each other, so we will keep both.

```{r}
active_sigs <- setdiff(active_sigs, c("Node34neg", "Node50pos", "Node34pos"))
```
